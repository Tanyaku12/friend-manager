...
def add_friend():
    ...
    try:
        result = send_encrypted_request(
            "https://client.ind.freefiremobile.com/RequestAddingFriend", payload, jwt
        )
...
def remove_friend():
    ...
    try:
        result = send_encrypted_request(
            "https://client.ind.freefiremobile.com/RemoveFriend", payload, jwt
        )
        status = result["status_code"]
        if status == 200:
...
def accept_friend():
    ...
    url = "https://client.ind.freefiremobile.com/ConfirmFriendRequest"
    headers = GAME_HEADERS.copy()
    headers['Authorization'] = jwt if jwt.startswith("Bearer") else f"Bearer {jwt}"
    try:
...
def reject_friend():
    ...
    try:
        ...
        return jsonify(OrderedDict([("success", False), ("error", "UID must be a number")])), 400
    url = "https://client.ind.freefiremobile.com/DeclineFriendRequest"
    headers = GAME_HEADERS.copy()
    headers['Authorization'] = jwt if jwt.startswith("Bearer") else f"Bearer {jwt}"
    try:
        binary_payload = smart_encode({"1": uid_int})
